...
function altitude_css() {
	...
	$opts = apply_filters( 'mono_images', array( '1', '2', '3', '4', '5', '6' ) );

	$settings = array();

	foreach( $opts as $opt ){
		$settings[$opt]['image'] = preg_replace( '/^https?:/', '', get_option( $opt .'-mono-image', sprintf( '%s/images/bg-%s.jpg', get_stylesheet_directory_uri(), $opt ) ) );
	}
